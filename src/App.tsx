...
function TabNav({ tab, setTab, variant }: { tab: string; setTab: (id: any) => void; variant: "bar" | "card" }) {
  return (
    <nav className={variant === "bar" ? "flex gap-1" : "mb-6 flex gap-1 rounded-xl border border-slate-200 bg-white p-1"}>
      {TABS.map((t) => (
        <button
          key={t.id}
          onClick={() => setTab(t.id)}
          className={
            variant === "bar"
              ? `rounded-lg px-3 py-1.5 text-sm font-medium transition ${tab === t.id ? "bg-slate-900 text-white" : "text-slate-600 hover:bg-slate-100"}`
              : `flex-1 rounded-lg px-3 py-2 text-sm font-medium transition ${tab === t.id ? "bg-slate-900 text-white" : "text-slate-600 hover:bg-slate-50"}`
          }
        >
          {t.label}
        </button>
...
export function App() {
  ...
  // Slim global top bar — lets you switch demos without competing with each demo's own chrome.
  const topbar = (
    <div className="sticky top-0 z-30 h-12 border-b border-slate-200 bg-white/85 backdrop-blur">
      <div className="mx-auto flex h-full max-w-[1400px] items-center justify-between gap-4 px-4">
        <span className="text-sm font-semibold text-slate-900">Building&nbsp;Blocks</span>
        <TabNav tab={tab} setTab={setTab} variant="bar" />
      </div>
    </div>
  );

  // The Design standard tab IS a documentation site — render it full-bleed under the top bar.
  if (tab === "design") {
    return (
      <div style={{ ["--ds-topbar" as string]: "48px" }}>
        {topbar}
        <DesignStandardDemo />
      </div>
    );
  ...
  // The other demos keep the focused, centered card layout.
  return (
    <div>
      {topbar}
